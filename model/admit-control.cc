...
TypeId AdmitControl::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::AdmitControl")
    ...
    .AddAttribute ("HighCapacityRbMultiplier",
                   "Extra RB margin factor applied to high-capacity traffic during admission gating. "
                   "1.0 fully trusts the CQI-driven requiredRbs from the scheduler (tight alignment).",
                   DoubleValue (1.5),
                   MakeDoubleAccessor (&AdmitControl::m_highCapacityRbMultiplier),
                   MakeDoubleChecker<double> (1.0))
    .AddAttribute ("ConsumerUlRbMargin",
                   "Extra RB margin added for consumer-grade terminals on the uplink (weaker UL capability).",
                   DoubleValue (0.15),
                   MakeDoubleAccessor (&AdmitControl::m_consumerUlRbMargin),
                   MakeDoubleChecker<double> (0.0))
    .AddAttribute ("EmergencyReservationRatio",
                   "Reserved RBs ratio for emergency services (0.0-1.0). "
                   "0603 增量: 兼容按比例预留应急资源的配置入口。",
                   DoubleValue (0.2),
...
AdmitControl::AdmitControl ()
  : m_emergencyReservedRbs (1),
    ...
    m_highCapacityRbMultiplier (1.5),
    m_consumerUlRbMargin (0.15),
                                         ...
                                         bool isUplink)
{
  ...
  const uint32_t availableRbs = GetAvailableRbs (targetBeamId, priority, isUplink);//根据业务优先级和预留策略计算当前可用 RB
  const uint32_t guaranteedAvailableRbs =
    GetGuaranteedAvailableRbs (targetBeamId, priority, isUplink);
  const bool requiresOpportunisticHeadroom =
    (priority == ServicePriority::PRIORITY_DATA ||
     priority == ServicePriority::PRIORITY_BEST_EFFORT) &&
    effectiveRequiredRbs > guaranteedAvailableRbs;
  ...
  // 4. 检查资源利用率阈值
  // 利用率分母统一取 ResourceManager 的每波束预算(按方向), 而非 BeamResourceStatus
  // 的独立默认值, 避免分母与真实预算解耦。
  const uint32_t beamBudgetRbs = GetBeamBudgetRbs (isUplink);
  const uint32_t physicalRemainingBeforeAdmit = GetPhysicalRemainingRbs (targetBeamId, isUplink);//当前物理层还剩多少 RB
  //beamBudgetRbs - physicalRemainingBeforeAdmit：当前已经使用了多少 RB 再加上 effectiveRequiredRbs，得到接入后的使用量
  const uint32_t usedAfterAdmit = beamBudgetRbs > physicalRemainingBeforeAdmit ?
                                  (beamBudgetRbs - physicalRemainingBeforeAdmit + effectiveRequiredRbs) :
                                  effectiveRequiredRbs;
  double utilizationAfterAdmit = static_cast<double>(usedAfterAdmit) / std::max (1u, beamBudgetRbs);//接入后的资源利用率
  //根据业务类型、终端类型、方向动态计算准入阈值
  const double admissionThreshold = GetAdmissionThreshold (priority, utType, trafficType, isUplink);
  if (requiresOpportunisticHeadroom)
    {
      NS_LOG_INFO ("[AdmitControl] Opportunistic-only headroom on Beam " << targetBeamId
                   << " | Required=" << effectiveRequiredRbs
                   << " | Guaranteed=" << guaranteedAvailableRbs
                   << " | Schedulable=" << availableRbs);
                                            ...
                                            bool isUplink)
{
  ...
  //重新计算目标波束在接入该 UE 后的资源利用率(分母统一用 RM 预算, 按方向)
  const uint32_t beamBudgetRbs = GetBeamBudgetRbs (isUplink);
  const uint32_t targetRemaining = GetPhysicalRemainingRbs (targetBeamId, isUplink);
  const uint32_t effectiveRequiredRbs =
    CalculateEffectiveRequiredRbs (targetBeamId, priority, utType, trafficType, requiredRbs, isUplink);
  const uint32_t targetUsedAfter = beamBudgetRbs > targetRemaining ?
                                   (beamBudgetRbs - targetRemaining + effectiveRequiredRbs) :
                                   effectiveRequiredRbs;
  double targetUtilization = static_cast<double>(targetUsedAfter) / std::max (1u, beamBudgetRbs);
      ...
      for (uint32_t altBeam : alternatives) {
          if (altBeam != targetBeamId) {
              ...
              if (altIt != m_beamResources.end ()) {
                  ...
                  const uint32_t altUsedAfter = beamBudgetRbs > altRemaining ?
                                                (beamBudgetRbs - altRemaining + effectiveRequiredRbs) :
                                                effectiveRequiredRbs;
                  double altUtil = static_cast<double>(altUsedAfter) / std::max (1u, beamBudgetRbs);//计算候选波束接入后的利用率
...
// 0603 增量: 4 参精简重载, 供 ntn-gw-rrc 调用。
// 内部委托给完整版 CanHandoverUe, 缺失的 utType/trafficType 取默认值。
AdmitDecision AdmitControl::CanHandoverUe (uint32_t sourceBeamId, uint32_t targetBeamId,
                                            ServicePriority priority, uint32_t requiredRbs)
{
  NS_LOG_FUNCTION (this << sourceBeamId << targetBeamId << (uint32_t)priority << requiredRbs);
  return CanHandoverUe (sourceBeamId, targetBeamId, priority,
                        UT_CONSUMER, TRAFFIC_DATA, requiredRbs, false);
}

// 0603 增量: 地面服务对象准入决策
AdmitDecision
AdmitControl::CanAdmitGroundUe (uint32_t groundId,
                                ServicePriority priority,
                                uint32_t requiredRbs) const
{
  NS_LOG_FUNCTION (this << groundId << (uint32_t)priority << requiredRbs);

  auto availIt = m_groundAvailability.find (groundId);
  const bool available = (availIt == m_groundAvailability.end ()) ? true : availIt->second;
  if (!available)
    {
      NS_LOG_WARN ("[AdmitControl] Ground " << groundId << " is disabled");
      return AdmitDecision::ADMIT_REJECTED;
    }

  auto capIt = m_groundCapacity.find (groundId);
  const uint32_t capacity = (capIt == m_groundCapacity.end ()) ? UINT32_MAX : capIt->second;
  auto activeIt = m_groundActiveUes.find (groundId);
  const uint32_t active = (activeIt == m_groundActiveUes.end ()) ? 0 : activeIt->second;

  if (active >= capacity)
    {
      NS_LOG_WARN ("[AdmitControl] Ground " << groundId << " capacity reached"
                   << " | active=" << active << " | capacity=" << capacity);
      return AdmitDecision::ADMIT_QUEUE;
    }

  NS_LOG_INFO ("[AdmitControl] Ground " << groundId << " admission OK"
               << " | active=" << active << " | capacity=" << capacity
               << " | requiredRbs=" << requiredRbs);
  return AdmitDecision::ADMIT_OK;
}
                                      ...
                                      UtType utType, TrafficType trafficType, Vector position)
{
      ...
      if (oldBeamId == beamId)//如果旧波束和新波束相同，说明不是切换，只是更新 UE 信息
        {
          ...
          existingUeIt->second.currentServiceObject =
            {ServiceObjectType::SERVICE_OBJECT_SAT_BEAM, beamId};
  ...
  ueInfo.currentServiceObject = {ServiceObjectType::SERVICE_OBJECT_SAT_BEAM, beamId};
...
// 0603 增量: 注册UE到地面服务对象
void
AdmitControl::RegisterUeToGround (uint16_t rnti, uint32_t groundId, ServicePriority priority,
                                  UtType utType, TrafficType trafficType, Vector position)
{
  NS_LOG_FUNCTION (this << rnti << groundId << (uint32_t)priority);

  UeContextInfo ueInfo;
  ueInfo.rnti = rnti;
  ueInfo.currentBeamId = 0;
  ueInfo.currentServiceObject = {ServiceObjectType::SERVICE_OBJECT_GROUND, groundId};
  ueInfo.priority = priority;
  ueInfo.utType = utType;
  ueInfo.trafficType = trafficType;
  ueInfo.position = position;
  ueInfo.latestDlCqi = 7.0;
  ueInfo.latestUlCqi = 7.0;
  ueInfo.latestCqi = 7.0;
  ueInfo.requiredRbs = (priority == ServicePriority::PRIORITY_EMERGENCY) ? 1 :
                       (priority == ServicePriority::PRIORITY_VOICE ? 2 : 1);
  ueInfo.connectedTime = Simulator::Now ();
  m_ueContextMap[rnti] = ueInfo;

  m_groundActiveUes[groundId]++;
  if (m_groundAvailability.find (groundId) == m_groundAvailability.end ())
    {
      m_groundAvailability[groundId] = true;
    }

  NS_LOG_INFO ("[AdmitControl] UE " << rnti << " registered to Ground " << groundId
               << " | Active UEs: " << m_groundActiveUes[groundId]);
}
...
// 0603 增量: 从地面服务对象注销UE
void
AdmitControl::UnregisterUeFromGround (uint16_t rnti, uint32_t groundId)
{
  NS_LOG_FUNCTION (this << rnti << groundId);
  ...
  auto activeIt = m_groundActiveUes.find (groundId);
  if (activeIt != m_groundActiveUes.end () && activeIt->second > 0)
    {
      activeIt->second--;
    }

  NS_LOG_INFO ("[AdmitControl] UE " << rnti << " unregistered from Ground " << groundId
               << " | Active UEs: "
               << ((activeIt != m_groundActiveUes.end ()) ? activeIt->second : 0));
}
                                    ...
                                    uint32_t requiredRbs)
{
  ...
  if (ueIt != m_ueContextMap.end ()) {
      ...
      ueIt->second.latestCqi = latestDlCqi;  //兼容字段同步
                                                         ...
                                                         bool isUplink)
{
  ...
  for (auto& beamPair : m_beamResources) {
      ...
      const double totalRbs = std::max (1u, GetBeamBudgetRbs (isUplink));//总 RB(RM 预算真源)，至少为 1，防止除 0
...
// 每波束总RB的单一真源: 绑定 ResourceManager 时按方向取其 Dl/Ul 预算,
// 否则回退到本地默认 (无 RM 的纯 AdmitControl 单测路径)。
uint32_t AdmitControl::GetBeamBudgetRbs (bool isUplink) const
{
  if (m_resourceManager != nullptr)
    {
      return isUplink ? m_resourceManager->GetUlBeamBudgetRbs ()
                      : m_resourceManager->GetDlBeamBudgetRbs ();
    }
  return isUplink ? m_totalUlRbs : m_totalDlRbs;
}
...
void
AdmitControl::SetGroundAvailability (uint32_t groundId, bool available)
{
  m_groundAvailability[groundId] = available;
  NS_LOG_INFO ("[AdmitControl] Ground " << groundId << " availability set to " << available);
}

bool
AdmitControl::IsGroundAvailable (uint32_t groundId) const
{
  auto it = m_groundAvailability.find (groundId);
  if (it == m_groundAvailability.end ())
    {
      return true;
    }
  return it->second;
...
void
AdmitControl::SetGroundCapacity (uint32_t groundId, uint32_t maxUes)
{
  m_groundCapacity[groundId] = maxUes;
  NS_LOG_INFO ("[AdmitControl] Ground " << groundId << " capacity set to " << maxUes);
}
...
//
// 语义修正(项 1): 这是某业务"准入机会可用上限"(本次准入实际能服务到多少 RB)。
// 关键区分:
//   - 预留量(m_emergencyReservedRbs / m_voiceReservedRbs / m_emergencyBurstCapRbs)是
//     "预留下限保证"——只用于在扣减**其它更低优先级**业务的可用时生效(见
//     GetGuaranteedAvailableRbs 的 DATA/BEST_EFFORT 分支), 不应反向把高优先级自身
//     的可用钳到这些小数值。
//   - 因此应急/语音的准入可用 = 物理剩余(语音还需让出应急保护份额, 仅作排序下限,
//     不再 min 到 m_voiceReservedRbs); 空/低载波束下它们能拿到所需(≤物理剩余)。
...
//计算基础可用 RB (每波束总RB统一取 RM 预算真源, 按方向)
  uint32_t totalRbs = GetBeamBudgetRbs (isUplink);
  uint32_t availableRbs = GetPhysicalRemainingRbs (beamId, isUplink);
  if (!isUplink)
    {
      availableRbs = (availableRbs > m_dlReservedRbs) ? (availableRbs - m_dlReservedRbs) : 0;
    }
  //计算应急保护份额(语音让出, 保证应急下限)
  const uint32_t protectedEmergency = std::min (m_emergencyReservedRbs, totalRbs);
//根据业务优先级返回可用 RB
  switch (priority)
    {
    case ServicePriority::PRIORITY_EMERGENCY:
      // 应急: 可用 = 物理剩余(不再死封到 burst cap=3; 预留量只是下限保证)。
      return availableRbs;
    case ServicePriority::PRIORITY_VOICE:
      // 语音: 让出应急保护份额后的物理剩余(不再死封到 voice reserved=2)。
      return (availableRbs > protectedEmergency) ? (availableRbs - protectedEmergency) : 0;
...
uint32_t
AdmitControl::GetGuaranteedAvailableRbs (uint32_t beamId,
                                         ServicePriority priority,
                                         bool isUplink)
{
  auto beamIt = m_beamResources.find (beamId);
  ...
  const uint32_t totalRbs = GetBeamBudgetRbs (isUplink);
  uint32_t availableRbs = GetPhysicalRemainingRbs (beamId, isUplink);
  if (!isUplink)
    {
      availableRbs = (availableRbs > m_dlReservedRbs) ? (availableRbs - m_dlReservedRbs) : 0;
...
{
//直接问ResourceManager当前物理剩余 RB(每波束预算的真源)
  if (m_resourceManager != nullptr)
    {
      return m_resourceManager->GetRemainingRbs (beamId, isUplink);
    }
//没有 ResourceManager 时使用本地估算; 每波束总RB按方向取本地默认。
  auto beamIt = m_beamResources.find (beamId);
  const uint32_t totalRbs = GetBeamBudgetRbs (isUplink);
  if (beamIt == m_beamResources.end ())
    {
...
//计算有效需求和剩余资源
//每波束总RB统一取 ResourceManager 的预算真源(按方向), 源/目标同方向预算一致。
  const uint32_t budgetRbs = GetBeamBudgetRbs (isUplink);
//计算该 UE 的有效 RB 需求
  const uint32_t effectiveRequiredRbs =
    CalculateEffectiveRequiredRbs (targetBeamId, priority, utType, trafficType, requiredRbs, isUplink);
...
//源波束可用比例 = 源波束剩余 RB / 源波束总 RB
  const double sourceAvailableRatio =
    budgetRbs > 0 ? static_cast<double> (sourceRemaining) / budgetRbs : 0.0;
//目标波束接入该 UE 后的资源余量比例
...
//(源波束已用 = 预算 - 剩余, 防止剩余>预算时的无符号下溢)
  const uint32_t sourceUsed = (budgetRbs > sourceRemaining) ? (budgetRbs - sourceRemaining) : 0;
  const double sourceReliefRatio =
    budgetRbs > 0 ?
    std::min<uint32_t> (effectiveRequiredRbs, sourceUsed) /
      static_cast<double> (budgetRbs) :
    0.0;
//计算最终收益
...
//更新保障资源和应急资源
  ...
  // 每波束总RB统一以 ResourceManager 的 DL 预算为真源(统计口径取 DL)。
  const uint32_t budgetRbs = GetBeamBudgetRbs (false);
  // BeamResourceStatus::totalRbs 仅作冗余镜像, 与真源保持一致, 供外部读取展示。
  beamStatus.totalRbs = budgetRbs;

  // 固定业务策略：应急默认1 PRB，语音默认2 PRB，合计3 PRB 刚性预留。
  beamStatus.guaranteedRbs = std::min (budgetRbs, m_emergencyReservedRbs + m_voiceReservedRbs);

  // 应急业务默认刚性预留与突发上限分开建模。
  beamStatus.emergencyRbs = std::min (budgetRbs, m_emergencyReservedRbs);

  // 计算总利用率
  const uint32_t remaining = GetPhysicalRemainingRbs (beamId, false);
  beamStatus.usedRbs = (budgetRbs > remaining) ? (budgetRbs - remaining) : 0;
  beamStatus.utilizationRatio = static_cast<double>(beamStatus.usedRbs) / std::max (1u, budgetRbs);
}
//业务类型映射优先级
...
AdmitControl::CalculateEffectiveRequiredRbs (uint32_t beamId,
                                             ...
                                             bool isUplink) const
{
  // 有效需求的上限统一以 ResourceManager 的每波束预算为真源(按方向)。
  const uint32_t totalRbs = GetBeamBudgetRbs (isUplink);
//根据业务类型设置需求倍率
  ...
  switch (trafficType)
    ...
    case TRAFFIC_VOICE:
      ...
      break;
    case TRAFFIC_HIGH_CAPACITY:
      multiplier = m_highCapacityRbMultiplier;
      break;
...
//消费级终端上行增加需求惩罚 消费级终端上行能力较弱，因此准入控制中要更保守
  if (isUplink && utType == UT_CONSUMER &&
    ...
    {
      multiplier += m_consumerUlRbMargin;
    }
