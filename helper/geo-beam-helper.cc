 ...
 *
 * 波束分配:
 *   f1 (2172.5 MHz): Beam 1, Beam 5 (复用)
 *   f2 (2177.5 MHz): Beam 2, Beam 6 (复用)
 *   f3 (2182.5 MHz): Beam 3, Beam 7 (复用)
...
const std::vector<BeamFrequencyConfig> g_4ColorFrequencyConfig = {
  ...
  {5, 2172.5e6, 5e6, 45230, 1},   //!< Beam 5 -> f1 (复用)
  {6, 2177.5e6, 5e6, 45240, 2},   //!< Beam 6 -> f2 (复用)
  {7, 2182.5e6, 5e6, 45250, 3}    //!< Beam 7 -> f3 (复用)
};
...
GeoBeamHelper::GeoBeamHelper ()
{
  ...
  m_bandwidthHz = 35e6;   // 统一为总名义带宽35MHz
  ...
  if (mode == 4)
    {
      ...
      NS_LOG_INFO ("Set to 4-color frequency reuse: 5MHz per color group, 35MHz nominal total beam bandwidth");
    }
  else if (mode == 7)
    {
  ...
  if (m_reuseMode == FrequencyReuseMode::FR1_4COLOR)
    {
      ...
      NS_LOG_INFO ("Nominal total beam bandwidth: 35 MHz (7 beams x 5 MHz)");
      NS_LOG_INFO ("Nominal total PRB baseline: 175 PRB (7 beams x 25 PRB)");
      NS_LOG_INFO ("Bandwidth per color group: 5 MHz (25 PRB per carrier)");
      NS_LOG_INFO ("Color 1 (f1=2172.5MHz): Beams 1, 5");
      NS_LOG_INFO ("Color 2 (f2=2177.5MHz): Beams 2, 6");
      NS_LOG_INFO ("Color 3 (f3=2182.5MHz): Beams 3, 7");
      NS_LOG_INFO ("Color 4 (f4=2187.5MHz): Beam 4");
    }
  else
    {
