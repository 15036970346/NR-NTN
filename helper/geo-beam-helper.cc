...
const std::vector<BeamFrequencyConfig> g_4ColorFrequencyConfig = {
  {1, 2175.0e6, 7.5e6, 45240, 1},   //!< Beam 1 -> f1
  {2, 2180.0e6, 7.5e6, 45260, 2},   //!< Beam 2 -> f2
  {3, 2185.0e6, 7.5e6, 45280, 3},   //!< Beam 3 -> f3
  {4, 2190.0e6, 7.5e6, 45300, 4},   //!< Beam 4 -> f4
  {5, 2175.0e6, 7.5e6, 45240, 1},   //!< Beam 5 -> f1 (复用)
  {6, 2180.0e6, 7.5e6, 45260, 2},   //!< Beam 6 -> f2 (复用)
  {7, 2185.0e6, 7.5e6, 45280, 3}    //!< Beam 7 -> f3 (复用)
};

/**
 * \brief 7色复用频率配置
 * \details
 * - 总带宽: 5 MHz (25 PRB, 子载波间隔15kHz)
...
GeoBeamHelper::GeoBeamHelper ()
{
  ...
  m_bandwidthHz = 30e6;   // 上下行总带宽30MHz
  ...
  if (mode == 4)
    {
      ...
      NS_LOG_INFO ("Set to 4-color frequency reuse: 7.5MHz (40PRB) per beam group");
    }
  else if (mode == 7)
    {
  ...
  if (m_reuseMode == FrequencyReuseMode::FR1_4COLOR)
    {
      ...
      NS_LOG_INFO ("Bandwidth per beam group: 7.5 MHz (40 PRB)");
